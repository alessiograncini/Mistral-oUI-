...
function parseURL(req) {
    ...
    if (typeof proto !== "string" || typeof host !== "string") {
        try {
            return new URL(relativeUrl, stdEnv.process.env.UPLOADTHING_URL);
        } catch (e) {
...
const createRouteHandler = (opts)=>{
    ...
    router.post("/", async (req, res)=>{
        ...
        if (response instanceof shared.UploadThingError) {
            res.status(shared.getStatusCodeFromError(response));
            res.setHeader("x-uploadthing-version", version);
...
exports.createUploadthingExpressHandler = createUploadthingExpressHandler;
