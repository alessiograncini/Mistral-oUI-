 ...
 */ const createUTReporter = (cfg)=>{
    return async (type, payload)=>{
        ...
        if (!response.ok) {
            const error = await shared.UploadThingError.fromResponse(response);
            throw error;
        }
        const jsonOrError = await shared.safeParseJSON(response);
...
const uploadFilesInternal = async (endpoint, opts)=>{
    ...
    const fileUploadPromises = s3ConnectionRes.map(async (presigned)=>{
        ...
        let serverData = null;
        if (!opts.skipPolling) {
            serverData = await shared.withExponentialBackoff(async ()=>{
